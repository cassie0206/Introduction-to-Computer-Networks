...
class SimpleController(app_manager.RyuApp):
    ...
    def flow_removed_handler(self, ev):
        ...
        global bw_con2
        if msg.priority==1:
            bw_sc+=(msg.packet_count)*(msg.byte_count)
        elif msg.priority==2:
            bw_con1+=(msg.packet_count)*(msg.byte_count)
        else:
            bw_con2+=(msg.packet_count)*(msg.byte_count)
        parser = dp.ofproto_parser
        match = parser.OFPMatch()
        actions = [parser.OFPActionOutput(ofp.OFPP_CONTROLLER, ofp.OFPCML_NO_BUFFER)]
        if bw_sc!=0 and bw_con1!=0 and bw_con2!=0:
            if bw_sc>bw_con1 and bw_sc>bw_con2:
                print('path3(SimpleController)')
                self.add_flow(
                    datapath=dp,
                    priority=1,
                    match=match,
            ...
            elif bw_con1>bw_sc and bw_con1>bw_con2:
                print('path1(Controller1)')
                self.add_flow(
                    datapath=dp,
                    priority=2,
                    match=match,
            ...
            elif bw_con2>bw_sc and bw_con2>bw_con1:
                print('path2(Controller2)')
                self.add_flow(
                    datapath=dp,
                    priority=3,
                    match=match,
